...
export class Joystick extends Component {
  ...
  _touchMoveEvent(event: EventTouch) {
    ...
    if (this.radius > distance) {
      ...
      //this.dot.angle=-misc.radiansToDegrees(Math.atan2(moveVec.normalize().y, moveVec.normalize().x)) - 90;
      speedType = SpeedType.FAST;
    }
    this.bg.angle=misc.radiansToDegrees(Math.atan2(moveVec.normalize().y, moveVec.normalize().x)) - 90;
